...
- `mt-funding-liquidity-fire-sales` and `mt-value-traders-arbitrageurs` (reading 14) derive the same performance-sensitive-capital limits-to-arbitrage mechanism from primary sources; this card applies it to the fund creation/redemption break.
- `be-limits-of-arbitrage` (reading 10, already migrated) is the canonical Shleifer-2000 owner of the "arbitrageurs most constrained when opportunities are best" result; defer the core derivation there at migration.
