...
| `export_utterances.py` | Export the corpus as a `cacg.utterances.v1` JSONL stream for the native `kb ingest --format utterances` backend (no pdfium; sealed `locator_map.json`); empty utterances skipped + counted in the header. | `--self-test` |
| `emit_practitioner_cards.py` | Emit the Layer-B practitioner cards from `practitioner_cards.json`: every Chinese quote bound author-origin to exactly one chunk via the AC-3 resolver (pins + `seed_overrides.json` honored); grounding dual-cites via the AC-6 probe binder; thesis discipline; all-or-nothing. | `--self-test` |
| `faithfulness_lint.py` | The deck faithfulness QA gate over the WRITTEN cards (config-driven, expert-agnostic): G1 verdict/thesis link, G2 attribution re-resolution under the same pins+overrides, G3 risk-spine, G4 recurrence honesty, G5 dating, G6 naming bans, G7 dual-cite. Config: `faithfulness_config.json`. | `--self-test` / `--check` |
| `practitioner_cards.json` / `faithfulness_config.json` | The Layer-B card specs (id/title/summary/tags/thesis/body/citations + candidate_title links) / the per-deck faithfulness config (author marker, verifications path, risk-spine phrases, tags, naming bans, excluded titles). | (data) |
