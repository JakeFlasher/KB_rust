...
| `acquire_grounding_sources.py` | Acquires the free Tier-1 grounding sources (AC-6) per `grounding_sources_spec.json`: downloads each to a dated, SHA-pinned local PDF (HTML via `html_to_pdf.py`), writes the committed `grounding_sources.json` manifest (+ omission ledger), and reconciles `ingest_plan.json`. Idempotent (reuse on SHA match). | `--acquire [--refresh]` |
| `grounding_sources_spec.json` / `grounding_sources.json` | The grounding-source URL spec (with `known_unresolved`) / the acquisition manifest (URLs + SHAs + dates + page counts). | (data) |
