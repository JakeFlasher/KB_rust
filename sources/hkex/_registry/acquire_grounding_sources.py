    ...
    for s in spec:
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            if code != "200" or not tmp.is_file() or tmp.stat().st_size == 0:
                omissions.append({"source_id": sid, "url": url, "reason": f"HTTP {code} / empty"})
                continue
            raw = tmp.read_bytes()
            ...
            if typ == "pdf":
                if not raw.startswith(b"%PDF"):
                    omissions.append({"source_id": sid, "url": url, "reason": "not a PDF"})
                    continue
                pdf.write_bytes(raw)
            elif typ == "html":
                ...
                omissions.append({"source_id": sid, "url": url, "reason": f"unknown type {typ}"})
                continue
            acquired.append({
...
def main() -> int:
    ...
    ap.add_argument("--acquire", action="store_true", required=True)
    ...
    print(json.dumps({"source_count": m["source_count"],
                      ...
                      "omitted": [o["source_id"] for o in m["omission_ledger"]]},
