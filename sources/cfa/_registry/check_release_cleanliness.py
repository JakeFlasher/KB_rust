...
TAG = "v1-candidate"
CARDS_MANIFEST = "out/cfa/cards_manifest.json"
TRACKED_SMALL = [
    CARDS_MANIFEST,
    "out/cfa/summaries.json",
    "out/cfa/INDEX.md",
    "out/cfa/pdfium_provenance.json",
]
EXCLUDED = ["out/cfa/chunks_manifest.json"]
RECIPE_SCRIPTS = [
...
def run_checks(*, require_tag: bool) -> list[str]:
    ...
    disk_sidecars = len(glob.glob(str(REPO / "cards/cfa/**/*.history.jsonl"), recursive=True))
    tracked_sidecars = len([l for l in git("ls-files", "cards/cfa/").splitlines()
...
def _self_test() -> int:
    ...
    expect("cards_manifest.json is tracked", is_tracked("out/cfa/cards_manifest.json"))
    expect("chunks_manifest.json is NOT tracked", not is_tracked("out/cfa/chunks_manifest.json"))
    expect("chunks_manifest.json is ignored", is_ignored("out/cfa/chunks_manifest.json"))
    expect("a nonexistent path is not tracked", not is_tracked("out/cfa/__nope__.json"))
    ...
    expect("modified tracked file is dirty",
           release_dirty_entries([" M out/cfa/cards_manifest.json"]) != [])
