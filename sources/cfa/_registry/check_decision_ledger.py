...
def derive_disk_facts(repo: Path) -> dict:
    ...
    """
    reg = repo / "sources/cfa/_registry"
    ...
    cited = set()
    cards = [
        c for c in glob.glob(str(repo / "cards/cfa/**/*.md"), recursive=True)
...
def active_card_count(repo: Path) -> int | None:
    ...
    p = repo / "out/cfa/cards_manifest.json"
    if not p.is_file():
        return None
...
def run_checks(text: str, *, repo: Path, evidence_must_exist: bool = True,
    ...
    elif (not re.search(rf"\b{active}\b", text) or "quarantin" not in text.lower()):
        # The ledger must state the live active-card count AND address the notes-taint
        # quarantine (its v0 carve-out and its 2026-06 resolution to zero).
        failures.append(f"definition-of-done ({active} active; quarantine status) not stated")
...
def _self_test() -> int:
    ...
    impl_bad = good.replace("| Q3 | DECIDED | — | ruling 3 |",
                            "| Q3 | IMPLICITLY SETTLED | `out/cfa/does_not_exist_zzz.json` | ruling 3 |")
