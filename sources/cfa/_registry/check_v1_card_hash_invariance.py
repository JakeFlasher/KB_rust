...
and the active total must be 408 (402 baseline + 6 re-authored notes-taint cards
admitted in the 2026-06 quarantine-absorption).

The v0 baseline is RE-DERIVED from the committed `v0-candidate` tag's
`out/cfa_legacy/cards_manifest.json` (268 cards — the tag predates the
`cfa_legacy -> cfa` rename, so the baseline is read at its historical path); the
current state is read from the working-tree `out/cfa/cards_manifest.json`. The
allowlist of released cards permitted to change is
...
# The v0-candidate tag predates the cfa_legacy -> cfa rename, so its tree stores the
# manifest at the OLD path. Read the baseline at the tag's historical path.
MANIFEST_AT_TAG = "out/cfa_legacy/cards_manifest.json"
V0_TAG = "v0-candidate"
...
def run_checks() -> tuple[list[str], dict]:
    failures: list[str] = []
    base = hashes(git_show(V0_TAG, MANIFEST_AT_TAG))
