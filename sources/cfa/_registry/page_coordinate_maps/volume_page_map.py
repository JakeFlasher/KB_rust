...
Schema: cfa.page_coordinate_map.v2 (per-volume affine transforms).
...
def load_map(path: str | Path) -> dict:
    ...
    accepted = {
        "cfa.page_coordinate_map.v1",
        "cfa.page_coordinate_map.v2",
        "cfa.page_coordinate_map.v2.1",  # adds content_end_pdf_page + junk_pdf_pages_after
    }
    if sv not in accepted:
        raise ValueError(f"unknown schema_version: {sv}")
    if "volume_table" in doc and sv not in {"cfa.page_coordinate_map.v2", "cfa.page_coordinate_map.v2.1"}:
