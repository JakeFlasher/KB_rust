#!/usr/bin/env python3
"""Emit the 08_convertible_bonds slice of CFA legacy cacg.v0 cards.
...
Uses hand-curated citations from `cb_08_slice_curated_citations.json` produced
...
CARDS_DIR = ROOT / "cards/cfa_legacy/08_convertible_bonds"
LEGACY_REF_DIR = ROOT / "_legacy_reference/cfa_legacy/cards/08_convertible_bonds"
OUT = ROOT / "out/cfa_legacy"
CURATED = REGISTRY / "cb_08_slice_curated_citations.json"
...
def derive_tags(card_id: str) -> list[str]:
    ...
    if len(body) >= 2:
        return ["convertible-bonds", "-".join(body[:2])]
    if len(body) == 1:
        return ["convertible-bonds", body[0]]
...
def main() -> int:
    ...
    for card in curated["cards"]:
        ...
        legacy_md = LEGACY_ROOT / ".claude/knowledge/08_convertible_bonds" / f"{card_id}.md"
    ...
    if not args.plan_only:
        write_atomic(
            REGISTRY / "cb_08_slice_citation_plan.json",
            json.dumps({"schema_version": "cfa_legacy.cb_08_slice.v1", "cards": plan}, indent=2, sort_keys=True, ensure_ascii=False) + "\n",
        )
