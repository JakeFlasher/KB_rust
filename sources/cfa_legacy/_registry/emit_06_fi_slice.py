#!/usr/bin/env python3
"""Emit the 06_fixed_income_and_credit slice of CFA legacy cacg.v0 cards.
...
Uses hand-curated citations from `fi_06_slice_curated_citations.json` produced
...
CARDS_DIR = ROOT / "cards/cfa_legacy/06_fixed_income_and_credit"
LEGACY_REF_DIR = ROOT / "_legacy_reference/cfa_legacy/cards/06_fixed_income_and_credit"
OUT = ROOT / "out/cfa_legacy"
CURATED = REGISTRY / "fi_06_slice_curated_citations.json"
...
def derive_tags(card_id: str) -> list[str]:
    ...
    if len(body) >= 2:
        return ["fixed-income", "-".join(body[:2])]
    if len(body) == 1:
        return ["fixed-income", body[0]]
    return ["fixed-income"]
...
def main() -> int:
    ...
    for card in curated["cards"]:
        ...
        legacy_md = LEGACY_ROOT / ".claude/knowledge/06_fixed_income_and_credit" / f"{card_id}.md"
    ...
    if not args.plan_only:
        write_atomic(
            REGISTRY / "fi_06_slice_citation_plan.json",
            json.dumps({"schema_version": "cfa_legacy.fi_06_slice.v1", "cards": plan}, indent=2, sort_keys=True, ensure_ascii=False) + "\n",
        )
