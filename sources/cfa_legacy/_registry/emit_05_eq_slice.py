#!/usr/bin/env python3
"""Emit the 05_equity slice of CFA legacy cacg.v0 cards.
...
Uses hand-curated citations from `eq_05_slice_curated_citations.json` produced
...
CARDS_DIR = ROOT / "cards/cfa_legacy/05_equity"
LEGACY_REF_DIR = ROOT / "_legacy_reference/cfa_legacy/cards/05_equity"
OUT = ROOT / "out/cfa_legacy"
CURATED = REGISTRY / "eq_05_slice_curated_citations.json"
...
def derive_tags(card_id: str) -> list[str]:
    ...
    if len(body) >= 2:
        return ["equity", "-".join(body[:2])]
    if len(body) == 1:
        return ["equity", body[0]]
    return ["equity"]
...
def main() -> int:
    ...
    for card in curated["cards"]:
        ...
        legacy_md = LEGACY_ROOT / ".claude/knowledge/05_equity" / f"{card_id}.md"
    ...
    write_atomic(
        REGISTRY / "eq_05_slice_citation_plan.json",
        json.dumps({"schema_version": "cfa_legacy.eq_05_slice.v1", "cards": plan}, indent=2, sort_keys=True, ensure_ascii=False) + "\n",
    )
