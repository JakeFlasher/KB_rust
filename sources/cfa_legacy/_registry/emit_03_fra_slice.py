#!/usr/bin/env python3
"""Emit the 03_financial_reporting_analysis slice of CFA legacy cacg.v0 cards.
...
Uses hand-curated citations from `fra_03_slice_curated_citations.json` produced
...
CARDS_DIR = ROOT / "cards/cfa_legacy/03_financial_reporting_analysis"
LEGACY_REF_DIR = ROOT / "_legacy_reference/cfa_legacy/cards/03_financial_reporting_analysis"
OUT = ROOT / "out/cfa_legacy"
CURATED = REGISTRY / "fra_03_slice_curated_citations.json"
...
def derive_tags(card_id: str) -> list[str]:
    ...
    if len(body) >= 2:
        return ["financial-reporting", "-".join(body[:2])]
    if len(body) == 1:
        return ["financial-reporting", body[0]]
...
def main() -> int:
    ...
    for card in curated["cards"]:
        ...
        legacy_md = LEGACY_ROOT / ".claude/knowledge/03_financial_reporting_analysis" / f"{card_id}.md"
    ...
    if not args.plan_only:
        write_atomic(
            REGISTRY / "fra_03_slice_citation_plan.json",
            json.dumps({"schema_version": "cfa_legacy.fra_03_slice.v1", "cards": plan}, indent=2, sort_keys=True, ensure_ascii=False) + "\n",
        )
