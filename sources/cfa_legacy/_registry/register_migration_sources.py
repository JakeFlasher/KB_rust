...
cfa_legacy ingest + authorization registries.

For every TEXT-LAYER source declared in each set's ``_sources.json`` this copies
the incoming PDF to ``sources/cfa_legacy/pdfs/<reading_id>/<source_id>.pdf``
...
gate-consumed ``out/cfa_legacy/source_matrix.json`` -- with the new reading keys
...
PDFS_DIR = ROOT / "sources/cfa_legacy/pdfs"
...
MATRIX_OUT = ROOT / "out/cfa_legacy/source_matrix.json"
...
def ingest_row(reading_id: str, source_id: str) -> dict[str, Any]:
    ...
    canonical = f"sources/cfa_legacy/pdfs/{reading_id}/{source_id}.pdf"
    out_dir = f"out/cfa_legacy/ingest_per_source/{source_id}"
    return {
...
def self_test() -> int:
    ...
    if row["canonical_path"] != "sources/cfa_legacy/pdfs/14_microstructure_and_trading/mt_harris_2003_trading_and_exchanges.pdf":
        failures.append("ingest_row canonical_path wrong")
    if row["out_dir"] != "out/cfa_legacy/ingest_per_source/mt_harris_2003_trading_and_exchanges":
