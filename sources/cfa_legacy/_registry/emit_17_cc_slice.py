#!/usr/bin/env python3
"""Emit the 17_cross_cutting slice of CFA legacy cacg.v0 cards.

Scope: 16 cards covering CFA L1 2022 Ethics R55-R57 + GIPS R59. All cards cite
cfa_2022_l1_combined Vol.6 via the existing offset table; no new offset map is
needed (the table at page_coordinate_maps/cfa_2022_l1_combined.json applies).

Uses hand-curated citations from `cc_17_slice_curated_citations.json` produced
by 2 parallel audit subagents. Emits all curated citations per card (primary +
supporting), mirroring the 11_RM fix-pass pattern.

The card cc-material-info-and-dissemination-delay reads its body from the
scrubbed _legacy_reference/ copy (7 bare-alias CFA_note_2 citations already
removed); all other cards read from upstream legacy KB at
.claude/knowledge/17_cross_cutting/.
"""
...
CARDS_DIR = ROOT / "cards/cfa_legacy/17_cross_cutting"
LEGACY_REF_DIR = ROOT / "_legacy_reference/cfa_legacy/cards/17_cross_cutting"
OUT = ROOT / "out/cfa_legacy"
CURATED = REGISTRY / "cc_17_slice_curated_citations.json"
...
def derive_tags(card_id: str) -> list[str]:
    """Generate tags from card_id: ['cfa-ethics', '<topic-suffix>']."""
    parts = card_id.split("-")
    if len(parts) >= 2:
        suffix = "-".join(parts[1:3]) if len(parts) > 2 else parts[1]
        return ["cfa-ethics", suffix]
    return ["cfa-ethics"]
...
def main() -> int:
    ...
    for card in curated["cards"]:
        ...
        legacy_md = LEGACY_ROOT / ".claude/knowledge/17_cross_cutting" / f"{card_id}.md"
    ...
    write_atomic(
        REGISTRY / "cc_17_slice_citation_plan.json",
        json.dumps({"schema_version": "cfa_legacy.cc_17_slice.v1", "cards": plan}, indent=2, sort_keys=True, ensure_ascii=False) + "\n",
    )
