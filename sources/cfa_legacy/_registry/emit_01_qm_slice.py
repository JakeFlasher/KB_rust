#!/usr/bin/env python3
"""Emit the 01_quantitative_methods slice of CFA legacy cacg.v0 cards.

Scope: 17 cards anchored on CFA L1 2022 Vol.1 R7 Linear Regression (13 cards)
plus 6 econometrics textbooks for 4 multi-source cards (ESL 2e, Greene 8e,
ISLP 2023, Tsay AFTS 2e, Tsay MTSA 2014, Wooldridge 8e — see
page_coordinate_maps/01_quantitative_methods.json for offsets).

Uses hand-curated citations from `qm_01_slice_curated_citations.json`.
Mirrors 05_EQ / 07_DERIV emitter pattern.
"""
...
CARDS_DIR = ROOT / "cards/cfa_legacy/01_quantitative_methods"
LEGACY_REF_DIR = ROOT / "_legacy_reference/cfa_legacy/cards/01_quantitative_methods"
OUT = ROOT / "out/cfa_legacy"
CURATED = REGISTRY / "qm_01_slice_curated_citations.json"
...
def derive_tags(card_id: str) -> list[str]:
    ...
    if len(body) >= 2:
        return ["quantitative-methods", "-".join(body[:2])]
    if len(body) == 1:
        return ["quantitative-methods", body[0]]
...
def main() -> int:
    ...
    for card in curated["cards"]:
        ...
        legacy_md = LEGACY_ROOT / ".claude/knowledge/01_quantitative_methods" / f"{card_id}.md"
    ...
    write_atomic(
        REGISTRY / "qm_01_slice_citation_plan.json",
        json.dumps({"schema_version": "cfa_legacy.qm_01_slice.v1", "cards": plan}, indent=2, sort_keys=True, ensure_ascii=False) + "\n",
    )
