...
This ledger closes the carried-forward open questions from `_research/28` §7 (Q1–Q12) and records the must-fix rulings for the CFA-legacy CACG **v0 release candidate**. It is the plan-record successor to `_research/28`. A deterministic checker (`sources/cfa/_registry/check_decision_ledger.py`) enforces that every Q1–Q12 carries a status and that no ruling contradicts DEC-2.
...
Definition of Done update (2026-06 Behavioral-Finance + Risk-Management template-deck merge): 500 active emitted cards (500 total, 0 quarantined) across the same 14 readings — the 408 post-absorption corpus plus 92 net-new additive cards on two EXISTING readings, 10_behavioral_finance (64 be-) and 11_risk_management (28 rm-). The 92 new cards pass the same trust gates (kb lint --all-readings exit 0 at 500; kb verify per-card 92/92; byte-reproducible frozen index re-baselined at 500); the corpus is ingested at 101 sources (87 + 14 new BF/RM PDFs) under the same Pdfium pin, with the prior 87 sources' chunk identities byte-stable. The additive release leaves all 408 pre-existing cards byte-unchanged (changed-vs-v0-baseline set is exactly the cross-link allowlist {be-limits-of-arbitrage}); EXPECTED_BASELINE = 268 stays immutable; the 48 Risk-Management deepenings to 15 live rm- cards land as a separate allowlisted commit. Schema stays cacg.v0. Faithfulness of the machine re-anchored / overridden quotes was independently reviewed (migration_faithfulness_review.json; 0 blocking after remediation).
...
**Q3 — IMPLICITLY SETTLED.** *(evidence: `sources/cfa/_registry/excluded_sources.json`)*
...
**Q5 — IMPLICITLY SETTLED.** *(evidence: `sources/cfa/_registry/snapshot.json`)*
...
RESOLVED: 08_CB contains exactly 50 cb-*.md active cards (verified: find cards/cfa/08_convertible_bonds -maxdepth 1 -name '*.md' | wc -l = 50). Unprefixed README.md is NOT a card and is NOT migrated to cards_manifest.json. The README is preserved as documentation-only in legacy_content_manifest per doc 28 §4.3. Doc 28 figure was stale; actual count 50 cards confirmed on disk and in manifest.

**Q7 — IMPLICITLY SETTLED.** *(evidence: `sources/cfa/_registry/card_migration_queue.json`)*
...
**Q8 — IMPLICITLY SETTLED.** *(evidence: `sources/cfa/_registry/card_migration_queue.json`)*
...
**Q10 — IMPLICITLY SETTLED.** *(evidence: `sources/cfa/_registry/snapshot.json`)*
...
At v0/v1, 8 legacy cards carried `notes_taint=true` in `card_migration_queue.json`; 2 of them (`cc-material-info-and-dissemination-delay`, `ec-currency-exchange-rates-and-parity`) were re-anchored to non-notes primary sources and emitted (flagged in the scope ledger as `notes_taint_flag_emitted_for_review`), and the remaining **6** were quarantined (AC-7).

**Update (2026-06 quarantine absorption — FUT-2 RESOLVED):** the remaining 6 were re-authored from non-notes primary sources (5 RM cards from McNeil QRM 2015; `rm-risk-objectives-and-tolerance` and `pm-tracking-error-and-active-risk` from CFA L1) and admitted to the active corpus. Their `notes_taint` flags are now `false`, the scope-ledger quarantine bucket is **0**, `QUARANTINE_IDS` is empty, and the active corpus is **408 active + 0 quarantined**. The structural Rule-9 guards remain enforced (no `notes_provenance`; no `notes//scripts/` citation paths; `rule9_canonical.md` pin intact), so absorbing the cards did not relax Rule 9.
...
- **FUT-2** — **RESOLVED (2026-06 quarantine absorption).** Re-author the 6 notes-tainted quarantined cards from non-notes sources. Source DEC: DEC-5. All 6 were re-authored from non-notes primary sources and admitted (rm-historical-simulation-var, rm-monte-carlo-var, rm-parametric-var, rm-sensitivity-versus-simulation from McNeil QRM 2015; rm-risk-objectives-and-tolerance + pm-tracking-error-and-active-risk from CFA L1), taking the active corpus from 402 to **408 with 0 quarantined**. `QUARANTINE_IDS` emptied; scope-ledger quarantine bucket 0; Rule-9 structural guards still enforced.
...
This is the final v0 decision ledger. All 12 doc-28 §7 questions are statused with rulings re-derived from on-disk artifacts (source_inventory.json, active card citations, PDF title-page/pdfinfo, the registries named per row), not from narrative summaries. The deterministic checker sources/cfa/_registry/check_decision_ledger.py (re-runnable, --self-test) gates this file.
