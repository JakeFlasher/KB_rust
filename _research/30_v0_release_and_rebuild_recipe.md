...
- **The small published index** — `out/cfa_legacy/cards_manifest.json` (~192 KB),
  `out/cfa_legacy/summaries.json` (~159 KB), `out/cfa_legacy/INDEX.md` (~32 KB).
...
- **Pdfium provenance** — `out/cfa_legacy/pdfium_provenance.json` (~620 B): build 7778
  (`chromium/7778`), the pinned text-extraction library. This is the Q4/AC-8
  reproducibility evidence; recording it is sufficient for v0 (no re-ingest).
...
|------|---------------|----------------|
| `out/cfa_legacy/chunks_manifest.json` (~85 MB) | Large ingest product (DEC-3: never commit). 57k+ chunks. | Rebuild recipe below. |
| `out/cfa_legacy/source_matrix.json` (~4 KB) | Regenerated by the SAME ingest+merge as chunks_manifest; kept with it for consistency (a half-tracked ingest output set is worse than a fully-regenerated one). | Rebuild recipe below (emitted by `merge_ingest_manifests.py`). |
| `out/cfa_legacy/lint_journal.jsonl` (~78 KB) | Transient — rewritten on every `run_corpus_gate.sh` run (`--journal`); not a release artifact. | Re-run the corpus gate. |
...
```bash
# Phase A — per-source ingest (chunk + hash every authorized source):
KB_FROZEN_CLOCK=1 python3 sources/cfa_legacy/_registry/run_ingest_per_source.py \
    --sources-root sources/cfa_legacy --out out/cfa_legacy --require-count 70

# Phase B — merge the per-source manifests into the corpus chunks_manifest +
# source_matrix (70 = the authorized source count):
